        		...
        		<tbody id="studentTable">
            		<?php
                		...
                		if(isset($_POST["ID"])){
                    		...
                    		$query = "SELECT Team.TeamCode, TeamName, StaffID, FirstName, LastName, Gender, Age, PhoneNumber FROM Team INNER JOIN WorksForStaff WHERE Team.TeamCode = WorksForStaff.TeamCode 
                    		AND Team.TeamCode = (SELECT TeamCode FROM AthleteInTeam WHERE AthleteID = $code3)";
                    		$result = mysqli_query($conn, $query);
                    		// put all our results into a html table
                    		while ($rows = mysqli_fetch_array($result)) {
                        		echo "<tr>";
