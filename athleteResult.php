        		...
        		<tbody id="studentTable">
            		<?php
                		...
                		if(isset($_POST["ID"])){
                    		...
                    		$query = "SELECT Event.EventName, Register.Place, Register.Score,Register.Time FROM Register,Event 
                    		WHERE Register.AthleteID = $code3 AND (Score IS NOT NULL OR Time IS NOT NULL) AND Register.EventCode = Event.EventCode" ;
                    		$result = mysqli_query($conn, $query);
                    		// put all our results into a html table
                    		while ($rows = mysqli_fetch_array($result)) {
                        		echo "<tr>";
