	...
	<div>
		...
		<div id="text">
        		...
        		<tbody id="studentTable">
            		<?php
                		// FILL TABLE WITH DATA ON CLICK
                    		...
                    		$query = "SELECT AthleteID, FirstName,LastName FROM Athlete" ;
    		...
    		<table class="table table-dark">
        		...
        		<tbody id="studentTable">
            		<?php
                		...
                		if(isset($_POST["ID"])){
                    		// get all our student data
                    		$code3 = mysqli_real_escape_string($conn, $_POST['ID']);
                    		$query = "SELECT Event.EventName, Venue.VenueCode, Venue.VenueName, HoldIn.Date 
                    		FROM ((Venue LEFT JOIN HoldIn ON Venue.VenueCode = HoldIn.VenueCode ) LEFT JOIN Event ON HoldIn.EventCode = Event.EventCode) LEFT JOIN Register ON Event.EventCode = Register.EventCode 
                    		WHERE AthleteID = $code3";
                    		$result = mysqli_query($conn, $query);
                    		// put all our results into a html table
                    		while ($rows = mysqli_fetch_array($result)) {
                        		echo "<tr>";
                        		echo "<td>".$rows["EventName"]."</td>";
                        		echo "<td>".$rows["VenueCode"]."</td>";
    		...
    		<form action="" method="post">
    			...
    			<input type="submit" value="Input AthleteID" />
    		</form>
		</div>
