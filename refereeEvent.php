	...
	<div>
		...
		<div id="text">
        		...
        		<tbody id="studentTable">
            		<?php
                		// FILL TABLE WITH DATA ON CLICK
                    		...
                    		$query = "SELECT RefereeID, FirstName,LastName FROM Referee" ;
                    		$result = mysqli_query($conn, $query);
                    		// put all our results into a html table
                    		while ($rows = mysqli_fetch_array($result)) {
                        		echo "<tr>";
                        		echo "<td>".$rows["RefereeID"]."</td>";
        		...
        		<tbody id="studentTable">
            		<?php
                		...
                		if(isset($_POST["ID"])){
                    		...
                    		$query = "SELECT EventName, EventCode, VenueName, Date FROM Temp WHERE EventCode IN (SELECT EventCode FROM Preside WHERE RefereeID = $code3)";
                    		$result = mysqli_query($conn, $query);
                    		// put all our results into a html table
                    		while ($rows = mysqli_fetch_array($result)) {
                        		echo "<tr>";
    		...
    		<form action="" method="post">
    			...
    			<input type="submit" value="Input RefereeID" />
    		</form>
		</div>
